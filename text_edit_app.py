...
class Window(Frame):
    def __init__(self, master=None):
        ...
        # Mainarea
        self.mainarea = Frame(master, bg='white', width=500, height=500)
        self.mainarea.pack(expand=True, fill='both', side='right')

        # Tabs
        self.tabs = Frame(self.mainarea, width=500, height=2, bg='white')
    ...
    def open_file(self):
        ...
        try:
            ...
            self.add_file_to_sidebar(self.filename)
            self.add_file_to_tab(self.filename)
        except Exception:
            print('Error')
    ...
    def add_file_to_sidebar(self, name):
        fname = pretty_filename(name)
        label = Label(self.sidebar, text=fname, width=15, height=1, anchor=NW, justify=LEFT)
        label.pack(fill=X)
        
        label.bind("<Button-1>", self.showFile)
    
    def add_file_to_tab(self, name):
        fname = pretty_filename(name)
        label = Label(self.tabs, text=fname, width=15, height=1, anchor=NW, justify=LEFT, bg='white')
        label.pack()
